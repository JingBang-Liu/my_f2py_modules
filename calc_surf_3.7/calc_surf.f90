MODULE my_calc_surf
    ...
    SUBROUTINE calc_dens(filename,input,n,output)
        ...
        cell_xyz_N = FLOOR(n/n_cells(1)/n_cells(2)*4.0d0)
    ...
    !!!! Subroutine that output density distribution for each
    !!!! particle
    SUBROUTINE dens_dist3(path,input,T,n,time,output)
        CHARACTER(LEN=*), INTENT(IN) :: path
        !!!! everything is in reduced unit
        !!!! input: simulation box boundaries (6), interested area boundaries (6)
        !!!!        cutoff, boundary (3) (0-f, 1-p)
        ...
        INTEGER, INTENT(IN), DIMENSION(:) :: time
        !!!! output: density distribution [timesteps, xcolu, ycolu, zcolu]
        REAL(REAL64), DIMENSION(T,n,4), INTENT(OUT) :: output
        REAL(REAL64), DIMENSION(:,:), ALLOCATABLE :: dens
        CHARACTER(LEN=300) :: filename
        CHARACTER(LEN=10) :: cTemp
        INTEGER :: i
        ...
        DO i=1,T
            ...
            ALLOCATE(dens(n,4))
            dens = 0
            CALL calc_dens(TRIM(ADJUSTL(filename)),input,n,dens)
            output(i,:,:) = dens
            DEALLOCATE(dens)
        END DO
    ...
    SUBROUTINE calc_surf(path,input,T,dens_b,xcolu,ycolu,time,output)
        ...
        DO i=1,T
            ...
            colu_xy = 0
            ALLOCATE(cell_xy(INT(n/xcolu/ycolu*4),xcolu,ycolu))
            cell_xy = 0
            DO j=1,indx
                ix = INT(FLOOR((dens(dens_liq_index(j),1)-input(7))/dx)) + 1
                iy = INT(FLOOR((dens(dens_liq_index(j),2)-input(9))/dy)) + 1
                IF (ix.GT.xcolu) THEN
                    ix = xcolu
                ELSE IF (ix.LT.1) THEN
                    ix = 1
    ...
    SUBROUTINE calc_surf_onefile(filename,input,T,n,dens_b,xcolu,ycolu,time,output)
        ...
        DO i=1,T
            ...
            DO j=1,n
                IF (input(14).EQ.1) THEN
                    IF ((posx(j).LT.input(7)) .OR. (posx(j).GT.input(8))) THEN
                        posx(j) = posx(j) - sign((input(8)-input(7)),(posx(j)-input(7)))
                    END IF
                END IF
                IF (input(15).EQ.1) THEN
                    IF ((posy(j).LT.input(9)) .OR. (posy(j).GT.input(10))) THEN
                        posy(j) = posy(j) - sign((input(10)-input(9)),(posy(j)-input(9)))
                    END IF
                END IF
                IF (input(16).EQ.1) THEN
                    IF ((posz(j).LT.input(11)) .OR. (posz(j).GT.input(12))) THEN
                        posz(j) = posz(j) - sign((input(12)-input(11)),(posz(j)-input(11)))
                    END IF
                END IF
            END DO
